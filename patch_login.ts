...
let content = fs.readFileSync('ilova/lib/screens/auth/login_screen.dart', 'utf8');
...
// 2. Replace _loginWithGoogle with Telegram logic
const telegramLogic = `
...
content = content.replace(/Future<void> _loginWithGoogle\(\) async \{[\s\S]*?\}\s*\}\s*@override/g, telegramLogic.trim() + '\n\n  @override');
...
content = content.replace(/\/\/ Google Login Button[\s\S]*?OutlinedButton\.icon\([\s\S]*?\}\),\s*\),\s*\),/m, telegramButton.trim() + ',');

fs.writeFileSync('ilova/lib/screens/auth/login_screen.dart', content);
